...
build_chart <- function(city_select_menu, cuisine_select_menu) {
  ...
  businesses <- businesses %>% 
    filter(location.city == city_select_menu & category == cuisine_select_menu) %>% 
    ...
    mutate(Name = paste0("<a href='", url, "' target='_blank' class='btn btn-primary'>", Name, "</a>")) %>%
    mutate(Image = paste0("<img src=\"", Image, "\" height=\"52\"></img>")) %>%
    select(Name, Rating, "Number of Reviews",
